...
Roles.filler.action = function(creep, state) {   //state = -2: startPos, state = 1: reversing path, state = 0 : static, state = 1 : advancing on path, state = 2 : endPos
    switch (state) {
        case -2 :
            return Roles.filler.startPosAction(creep);
            break;
        
    }
}

Roles.filler.startPosAction = function(creep) {
